...
- phase-0-foundation/ - Repo hygiene, .gitignore, README, branch protection baseline
...
- lab-notes/ - Cross-phase incidents and repo-wide process notes only. Per-phase postmortems live inside that phase's own postmortem/ folder.
...
8. 8. Postmortem - written to that phase's own postmortem/ folder: what happened, the actual root cause, the evidence, the guardrail now in place.
## Progression rule
...
- Produce a postmortem entry (in that phase's postmortem/ folder) that could hand off to a teammate with zero additional context.
...
Currently in progress: Phase 0 - Foundation, proven and documented. See
ROADMAP.md for the full status table across all phases, and each phase's
own postmortem/ folder for evidence.
